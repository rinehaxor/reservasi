...
const alertVariants = cva('relative w-full rounded-lg border p-4 [&>svg~*]:pl-7 [&>svg+div]:translate-y-[-3px] [&>svg]:absolute [&>svg]:left-4 [&>svg]:top-4 [&>svg]:text-foreground', {
   variants: {
      variant: {
         ...
         primary: 'border-destructive/50 text-white dark:border-destructive [&>svg]:text-white bg-green-500',
         outline: 'border-foreground/50 text-foreground dark:border-foreground [&>svg]:text-foreground',
      },
   },
   defaultVariants: {
      variant: 'default',
